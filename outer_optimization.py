...
def calcGradient(eta, phi, fee, feestar):
    D,V,K = phi.shape
    eta_sum = [0.0]*K
    eps = 0.005
    ...
    for i in range(len(d_arr)):
        ...
        '''
        M_grad[d][v] += sgn(fee[k,v] - feestar[k][v]) * max(0,(abs(fee[k,v] - feestar[k][v]) - eps))*((eta_sum[k] - eta[k][v])/eta_sum[k]**2) * phi[d, v, k]

        '''
            the  simple l2 risk function
        '''
        
        #M_grad[d][v] += (fee[k,v] - feestar[k][v])*((eta_sum[k] - eta[k][v])/(eta_sum[k]*eta_sum[k])) * phi[d, v, k]
...
##def update(eta, phi, feestar, M_0, M,it):
##    D,V,K = phi.shape
##    L = 600
##    L_d = 10
##    flag = True
...
##
##        M_grad[d] = M_grad[d]*min(temp*0.1, learning_rate*0.1)*(M[d] - M_grad[d] > 0)/it
##
####        for v in range(V):
####            if M_grad[d][v] < 0 :
...
def update(eta, phi, fee, feestar, M_0, M,it,momentum):
    D,V,K = phi.shape
    beta = 0.95
    L = 600
    L_d = 10
    ...
    #projection onto the set M
    M_grad = calcGradient(eta, phi, fee, feestar)
    ...
    return (M - ((50.0/(it))*momentum),momentum)
