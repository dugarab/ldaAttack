...
// New
void save_phi(char* filename, double*** phi3d, int num_docs, int voc, int num_topics)
{
    FILE* fileptr;
    int d,v,k;
    fileptr = fopen(filename, "w");
...
void run_em(char* start, char* directory, corpus* corpus)
{
  ...
  int d, n, k;
    ...
    // new
    phi3d = malloc(sizeof(double**)*(corpus->num_docs));
    for (d=0;d<corpus->num_docs;d++){
      phi3d[d] = malloc(sizeof(double *)*(corpus->num_terms));
      for (n = 0; n < corpus->num_terms; n++)
	phi3d[d][n] = malloc(sizeof(double) * NTOPICS);
    }
    for (d=0;d<corpus->num_docs;d++){
      for (n=0;n<corpus->num_terms;n++){
	for (k=0;k<NTOPICS;k++)
    ...
    while (((converged < 0) || (converged > EM_CONVERGED) || (i <= 2)) && (i <= EM_MAX_ITER))
    {
	    ...
	    document* docu = &(corpus->docs[d]);
            likelihood += doc_e_step(docu,
                                     var_gamma[d],
                                     phi,
                                     model,
                                     ss);
	    //new
	    for (n=0; n < docu->length; n++){
	      for (k=0; k<NTOPICS; k++)
		phi3d[d][docu->words[n]][k] = phi[n][k];
	    }
	    //end
        }
